  ...
  return (
    <div ref={root} className="site-shell">
      ...
      <main id="top">
        <section className="hero section-pad">
          <div className="hero-grid container">
            ...
            <div className="hero-visual">
              <div className="portrait-wrap"><img className="hero-portrait" src="/manus-storage/doctor-portrait_7d766e41.png" alt="Dr. Mohammad Anwarul Islam, Medicine Specialist and Neurologist" /></div>
        ...
        <section className="credential-strip"><div className="credential-inner container">{[["MBBS", "Mymensingh Medical College"], ["BCS (Health)", "Bangladesh Health Cadre"], ["FCPS", "Postgraduate Medicine"], ["MD", "Neurology Specialization"], ["BMDC", "Reg. A-61657"]].map(([a,b]) => <div className="credential" key={a}><strong>{a}</strong><span>{b}</span></div>)}</div></section>
        ...
        <section id="expertise" className="expertise section-pad"><div className="container"><div className="expertise-heading reveal"><SectionLabel number="03">SPECIALIST FOCUS</SectionLabel><h2>Neurology, approached<br /><i>with clinical depth.</i></h2><p>Consultation and assessment for a broad range of neurological symptoms and conditions.</p></div><div className="expertise-workspace"><div className="expertise-index">{expertise.map(([n, title], i) => <button key={n} className={activeExpertise === i ? "active" : ""} onClick={() => setActiveExpertise(i)}><span>{n}</span>{title}</button>)}</div><div className="expertise-detail"><div className="detail-count">{expertise[activeExpertise][0]} <span>/ 07</span></div><h3>{expertise[activeExpertise][1]}</h3><p>{expertise[activeExpertise][2]}</p><div className="detail-line"><span style={{ width: `${((activeExpertise + 1) / expertise.length) * 100}%` }} /></div><div className="detail-nav"><button onClick={() => setActiveExpertise(Math.max(0, activeExpertise - 1))}>Previous</button><button onClick={() => setActiveExpertise((activeExpertise + 1) % expertise.length)}>Next <ArrowUpRight size={15} /></button></div></div></div></div></section>

        <section className="medicine-band section-pad"><div className="container medicine-grid"><div className="reveal"><SectionLabel number="04">THE WIDER PICTURE</SectionLabel><h2>Beyond neurology,<br /><i>grounded in medicine.</i></h2></div><div className="medicine-copy reveal"><p>Medicine training provides a broad adult medical foundation for symptoms and conditions that may overlap or require thoughtful specialist assessment.</p><div className="medicine-list">{["General adult medicine", "Fever & infections", "Weakness & fatigue", "Hypertension-related medical problems", "Diabetes-related medical problems", "Chronic medical conditions"].map((item) => <span key={item}>{item}<ArrowUpRight size={14} /></span>)}</div></div></div></section>
